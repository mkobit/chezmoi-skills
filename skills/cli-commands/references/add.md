...
- `chezmoi add --exact --recursive DIR` on a nested directory (e.g., `~/.config/nvim`) marks all parent directories `exact_` too, so unrelated files under `~/.config` get removed on the next `apply`.
  Add a `.keep` file in the parent first: `chezmoi add ~/.config/.keep`, then add the subdirectory.
